...
class MyHomeClient {
    ...
    onMonitor(_frame) {
        // Connection delivers only complete, delimited frames.
        ...
        frames.forEach(function(frame) {
            ...
            /* WHO=18 — Instant power (W): *#18*<where>*113*<Val> */
            extract = frame.match(/^\*#18\*(\d+)\*113\*(\d+)$/);
            if (extract) {
                var where18 = parseInt(extract[1], 10);
                var watts = parseInt(extract[2], 10);
                if (this.parent != null && typeof(this.parent.onPower) == 'function') this.parent.onPower(where18, watts);
                debug(sprintf("POWER %s W ADDRESS %s", watts, where18));
            }

            /* WHO=18 — Total energy (Wh): *#18*<where>*51*<Val> */
            extract = frame.match(/^\*#18\*(\d+)\*51\*(\d+)$/);
            if (extract) {
                var where18t = parseInt(extract[1], 10);
                var wh = parseInt(extract[2], 10);
                if (this.parent != null && typeof(this.parent.onEnergyTotal) == 'function') this.parent.onEnergyTotal(where18t, wh);
                debug(sprintf("ENERGY %s Wh ADDRESS %s", wh, where18t));
            }
    ...
    getInstantPower(_where) {
        // Response: *#18*<where>*113*<Val>##
        this.command.send(sprintf("*#18*%s*113##", _where));
    }

    getEnergyTotal(_where) {
        // Response: *#18*<where>*51*<Val>##
        this.command.send(sprintf("*#18*%s*51##", _where));
    }
...
class MyHomeConnection {
    ...
    connect() {
        ...
        this.connection.on('data', (_in) => {
            ...
            this.receiveBuffer += _in.toString();
            var boundary;
            while ((boundary = this.receiveBuffer.indexOf("##")) !== -1) {
                var data = this.receiveBuffer.slice(0, boundary + 2).trim();
                this.receiveBuffer = this.receiveBuffer.slice(boundary + 2);
        ...
        this.connection.on('close', () => {
            ...
            this.isConnected = false;
            this.receiveBuffer = "";
            clearTimeout(this.disconnecthandle);
    ...
    /* Schedule at most one reconnect: a socket error is normally followed by
       close. Keep one pending retry for the connection. */
    _scheduleReconnect() {
